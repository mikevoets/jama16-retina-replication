...
mkdir -p "$eyepacs_dir/bin2/develop/0" "$eyepacs_dir/bin2/develop/1"
mkdir -p "$eyepacs_dir/bin2/validate/0" "$eyepacs_dir/bin2/validate/1"
...
echo "Gathering images for development set (0/2)"
distribute_images "$bin2_0" head "$bin2_0_tr_cnt" "$eyepacs_dir/bin2/develop/0/."

echo "Gathering images for development set (1/2)"
distribute_images "$bin2_1" head "$bin2_1_tr_cnt" "$eyepacs_dir/bin2/develop/1/."

echo "Gathering images for validation set (0/2)" 
distribute_images "$bin2_0" tail "$(expr $bin2_0_cnt - $bin2_0_tr_cnt)" "$eyepacs_dir/bin2/validate/0/."

echo "Gathering images for validation set (1/2)"
distribute_images "$bin2_1" tail "$(expr $bin2_1_cnt - $bin2_1_tr_cnt)" "$eyepacs_dir/bin2/validate/1/."

echo "Converting data set to tfrecords..."
git submodule update --init

python ./create_tfrecords/create_tfrecord.py --dataset_dir="$eyepacs_dir/bin2/develop" \
       --tfrecord_filename=eyepacs --num_shards=8 --validation_size=0.2 || \
    { echo "Submodule not initialized. Run git submodule update --init"; 
      exit 1; }

python ./create_tfrecords/create_tfrecord.py --dataset_dir="$eyepacs_dir/bin2/validate" \
